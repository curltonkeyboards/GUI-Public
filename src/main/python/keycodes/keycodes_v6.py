class keycodes_v6:
    kc = {
        ...
        # Old per-key chord progression keycodes REMOVED (504 keycodes: C_CHORDPROG1-B_CHORDPROG42)
        # Replaced by 20 CPROG_SLOT keycodes above
        ...
        "CLEAR_MENU": 0xCCAF,   # Clear/reset menu (ALL, MODIFIERS, LOOPS, QUICK BUILD, FACTORY)
        ...
        # Delay / Smart Chord / Dynamic Chord Quick Build
...
for x in range(128):
    ...
    keycodes_v6.kc["MI_MOD_PRESS_{}".format(x)] = keycodes_v6.kc["MI_MOD_PRESS_0"] + x
...
for c in range(6):       # 6 category groups
    for m in range(3):
        keycodes_v6.kc["DRUMLIVE_CAT_{}_{}".format(c, m)] = keycodes_v6.kc["DRUMLIVE_CAT_BASE"] + c * 3 + m
for v in range(12):      # 12 individual voices
    for m in range(3):
        keycodes_v6.kc["DRUMLIVE_VOICE_{}_{}".format(v, m)] = keycodes_v6.kc["DRUMLIVE_VOICE_BASE"] + v * 3 + m
...
for t in range(-64, 65):
    keycodes_v6.kc["MI_TRNS_SET_N{}".format(-t) if t < 0 else "MI_TRNS_SET_{}".format(t)] = 0xF380 + (t + 64)
...
for t in range(-64, 65):
    keycodes_v6.kc["MI_KS_TRNS_SET_N{}".format(-t) if t < 0 else "MI_KS_TRNS_SET_{}".format(t)] = 0xC4F0 + (t + 64)
    keycodes_v6.kc["MI_TS_TRNS_SET_N{}".format(-t) if t < 0 else "MI_TS_TRNS_SET_{}".format(t)] = 0xC578 + (t + 64)
...
for n in range(1, 11):
    keycodes_v6.kc["KEYSPLIT_PRESET_{}".format(n)] = 0xF420 + (n - 1)

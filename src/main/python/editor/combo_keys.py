...
A Combo Key is: hold one or more "held keys" (the 8 modifiers, Fn 1, Fn 2),
...
On the wire this is still a Vial key override entry (protocol/key_override.py):
    trigger        = tapped key
    replacement    = output key
    trigger_mods   = held modifiers
    suppressed_mods= held modifiers (the output is always sent clean)
    negative mods  = none
    layers         = all 12 layers, plus Fn 1 / Fn 2 in bits 12-13
    options        = the firmware's default activation events + the enabled bit
"""
import sip
...
# vial_key_override_entry_t.layers: bits 0-11 = layers, bits 12-13 = Fn held keys
LAYERS_ALL = 0x0FFF
FN_SHIFT = 12
FN1 = 1 << 0
FN2 = 1 << 1

# Firmware default activation events (ko_options_default): activate when the
# tapped key goes down, when a required held key goes down, or when a negative
# mod (none here) goes up.
...
def entry_to_model(ko):
    ...
    An all-zero entry (the firmware's "unused" state) reads as an enabled,
...
def model_to_entry(model):
    ...
    the firmware treats as empty — never as an enabled override with an empty
    trigger, which QMK would activate on every key press."""
...
class ComboKeyEntryUI(QObject):
    ...
    def _ensure_ui(self):
        ...
        self.title_label = QLabel("<b>Combo Key {}</b>".format(idx + 1))
